...
formulario.addEventListener("submit", function (event) {
  ...
  nuevaCelda.textContent = formularioAFormData.get("primerNombre");
  let nuevaCelda1 = insertarFilaRef.insertCell(1);
  nuevaCelda1.textContent = formularioAFormData.get("segundoNombre");
  let nuevaCelda2 = insertarFilaRef.insertCell(2);
  nuevaCelda2.textContent = formularioAFormData.get("primerApellido");
  let nuevaCelda3 = insertarFilaRef.insertCell(3);
  nuevaCelda3.textContent = formularioAFormData.get("segundoApellido");
...
let darColorNombre = (data1, data2) => {
  let promptValues = prompt(
    "¿En caso de querer remarcar el nombre con un color,escribalo en ingles,ej 'blue', 'red',etc?"
  );
...
let darColorApellido = (data1, data2) => {
  let promptValues = prompt(
    "¿En caso de querer remarcar el apellido con un color,escribalo en ingles,ej 'blue', 'red',etc?"
  );
  data1.style.backgroundColor = `${promptValues}`;
  data2.style.backgroundColor = `${promptValues}`;
  // data.style.background="red"
...
let coincienciaApellido = (
  apellidoOneFirstInt,//[0]
  apellidoTwoFirstInt,//[1]
  apellidoOneSecondInt,//[4]
  apellidoTwoSecondInt//[5]
) => {
  if (apellidoOneFirstInt.textContent == apellidoOneSecondInt.textContent) {
    alert(
      `Hubo coincidencia en el apellido ${apellidoOneFirstInt.textContent} `
    );
  ...
  if (apellidoTwoFirstInt.textContent == apellidoOneSecondInt.textContent) {
    alert(
      `Hubo coincidencia en el apellido ${apellidoTwoFirstInt.textContent} `
    );
...
boton.addEventListener("click", () => {
  let td = document.querySelectorAll("td");
  // console.log(td[0]);
  coincidenciaNombre(td[0], td[1], td[4], td[5]);
  coincienciaApellido(td[2], td[3], td[6], td[7]);
});
